...
def getROI(image, faceBox):
    ...
    if USE_SEGMENTATION:
        ...
        # print("Called USE Segmentation else")

        backgrndMask = np.full(image.shape, False, dtype=bool)  #True
        # print(backgrndMask)

        backgrndMask[y:y + h, x:x + w, :] = True  #False
        # print("After np.full")
        # print(backgrndMask)
    ...
    (x, y, w, h) = faceBox
    if REMOVE_EYES:
        backgrndMask[y + h * EYE_LOWER_FRAC: y + h * EYE_UPPER_FRAC, :] = False     #True
    if FOREHEAD_ONLY:
        backgrndMask[y + h * EYE_LOWER_FRAC:, :] = False  #True
    # print("ifnal")
    # print(backgrndMask)
    # roi = np.ma.array(image, mask=backgrndMask)  # Masked array
    roi = image * backgrndMask  # multiply `im` by the mask to zero out non-border pixels
    # print("roi\nroi\nroi")
    # print(roi)
...
def getBestROI(frame, faceCascade, previousFaceBox):
    ...
    if faceBox is not None:
        ...
        # Show rectangle
        # (x, y, w, h) = faceBox
        # cv2.rectangle(frame, faceBox, (255, 255, 255), 2)
...
while True:
    ...
    if heartRates:
        print(heartRates[-1]*60)
        roi= cv2.putText(roi, str(round(heartRates[-1]*60 , 2
                                        )) + " bpm", (100, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
    cv2.imshow('ROI', roi)
    cv2.waitKey(1)

# print(heartRates[-1],heartRates[-1]*60)
# print (videoFile)
